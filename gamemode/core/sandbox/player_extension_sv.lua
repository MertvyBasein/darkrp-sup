function PLAYER:CheckLimit(str)
	...
	if self:GetUserGroup() == "admin" or self:GetUserGroup() == "moderator" or self:GetUserGroup() == "headadmin" or self:GetUserGroup() == "manager" or self:GetUserGroup() == "superadmin" then
		c = rp.GetLimit(str) * 3
	end
...
function PLAYER:LimitHit(str)
	rp.Notify(self, NOTIFY_ERROR, 'Вы достигли # лимит!', str)
end

	...
	for _, ent in ipairs(DoorCache) do
		if IsValid(ent) then
			...
			if ent:DoorIsOwnable() then
				AddText({color_white, 'Арендовать [F2]'})
			...
			elseif not ent:DoorIsOwnable() then
				...
				if (ent:DoorGetGroup() ~= nil) then
					...
					if rp.teamDoors[group] then
						AddText({rp.teamDoors[group].Color, group})
					else
						AddText({color_white, group})
					end
				end
				...
				if IsValid(owner) then
					AddText({owner:GetTeamColor(), owner:Name()})
				end
				...
				if (ent:DoorGetCoOwners() ~= nil) then
					for k, co in ipairs(ent:DoorGetCoOwners()) do
						if IsValid(co) then
							AddText({co:GetTeamColor(), co:Name()}) -- TODO: FIX
						end

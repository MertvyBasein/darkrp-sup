...
function ENTITY:DoorCoOwnedBy(pl)
	if (self:DoorGetGroup() ~= nil) then return (rp.teamDoors[self:DoorGetGroup()][pl:Team()] or false) end
	if IsValid(self:DoorGetOwner()) and self:DoorGetOwner():GetOrg() and self:DoorOrgOwned() and (pl:GetOrg() == self:DoorGetOwner():GetOrg()) then return true end
...
function ENTITY:DoorGetColor()
	return (istable(self:GetNetVar('DoorData')) and (self:GetNetVar('DoorData').Color) or nil)
end
...
function ENTITY:DoorGetGroup()
	return (istable(self:GetNetVar('DoorData')) and (self:GetNetVar('DoorData').Group) or nil) or nil
end

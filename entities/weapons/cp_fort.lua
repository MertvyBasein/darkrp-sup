...
if CLIENT then
	SWEP.PrintName = "Инженерная панель"
	SWEP.Slot = 1
	SWEP.SlotPos = 1
	SWEP.DrawAmmo = false
...
SWEP.Fortifications = {
	{name = "Станковый пулемёт 1", model = "models/red orchestra 2/russians/weapons/maxim.mdl", ent = 'ent_weapon_maximpod'},
	{name = "Станковый пулемёт 2", model = "models/props_junk/TrafficCone001a.mdl", ent = 'ent_weapon_mg42pod'},
}
...
if SERVER then
	...
	hook.Add('CanUndo', 'cyphersky.enginer',function(ply, tbl)
		if tbl.NiceText == 'Объект (Раздатчик патрон)' then
			ply:SetNWInt('metahub_goammo', ply:GetNWInt('metahub_goammo') - 1)
		elseif tbl.NiceText == 'Объект (Станковый пулемёт 1)' then
			ply:SetNWInt('ent_weapon_maximpod', ply:GetNWInt('ent_weapon_maximpod') - 1)
		elseif tbl.NiceText == 'Объект (Станковый пулемёт 2)' then
			ply:SetNWInt('ent_weapon_mg42pod', ply:GetNWInt('ent_weapon_mg42pod') - 1)
		end
	end)

	hook.Add("OnPlayerChangedTeam", "cyphersky.enginer2", function(ply, old, new)
		if old == TEAM_CCCR4 or old == TEAM_REIX4 or old == TEAM_USA4 then
			-- for k,v in pairs(ents.FindByClass('metahub_goammo')) do
			-- 	if v.isPlayerPlacedFortification == ply then
			-- 		v:Remove()
			-- 		ply:SetNWInt('metahub_goammo', 0)
			-- 	end
			-- end
			for k,v in pairs(ents.FindByClass('ent_weapon_mg42pod')) do
				if v.isPlayerPlacedFortification == ply then
					v:Remove()
					ply:SetNWInt('ent_weapon_mg42pod', 0)
				end
			end
			for k,v in pairs(ents.FindByClass('ent_weapon_maximpod')) do
				if v.isPlayerPlacedFortification == ply then
					v:Remove()
					ply:SetNWInt('ent_weapon_maximpod', 0)
				end
			end
		end
	end)
	...
	hook.Add("KeyPress", "Alydus_KeyPress_HandleBuildTabletHologramFinished", function(ply, key)
		if key == IN_USE and IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "cp_fort" and ply:GetActiveWeapon().fortificationSelection then
			if (ply:GetEyeTrace().Entity and ply:GetEyeTrace().Entity:IsPlayer()) or ply:Crouching() or (IsValid(ply:GetEyeTrace().Entity) and table.HasValue(ply:GetActiveWeapon().FortificationsModelList, ply:GetEyeTrace().Entity:GetModel())) or hook.Call("Alydus.FortificationBuilderTablet.CanBuildFortification", ply) or (IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "cp_fort" and ply:GetActiveWeapon():GetNWBool("tabletBootup", false) == true) then
				...
				if ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'ent_weapon_maximpod' or ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'ent_weapon_mg42pod' then
					for k,v in pairs(ents.FindInSphere(ply:GetEyeTrace().HitPos - ply:GetEyeTrace().HitNormal * ply.fortificationHologram:OBBMins().z, 600)) do
						if v:GetClass() == 'ent_weapon_maximpod' or v:GetClass() == 'ent_weapon_mg42pod' then rp.Notify(ply,1,'Рядом уже стоит пулемет!') return end

					end
				end
				if ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'ent_weapon_m60pod' and ply:GetNWInt('ent_weapon_m60pod') >= 1 then rp.Notify(ply,1,'У вас уже стоит станковый пулемёт') return end
				if ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'metahub_goammo' and ply:GetNWInt('metahub_goammo') >= 1 then rp.Notify(ply,1,'У вас уже стоит раздатчик патронов') return end
				local fortification = ents.Create(ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"])
				fortification:SetAngles(Angle(100, ply:EyeAngles().y - 180, 0))
				...
				if ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'ent_weapon_maximpod' then
					ply:SetNWInt('ent_weapon_maximpod', ply:GetNWInt('ent_weapon_maximpod') + 1)
				elseif ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'ent_weapon_mg42pod' then
					ply:SetNWInt('ent_weapon_mg42pod', ply:GetNWInt('ent_weapon_maximpod') + 1)
				elseif ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["ent"] == 'metahub_goammo' then
					ply:SetNWInt('metahub_goammo', ply:GetNWInt('metahub_goammo') + 1)
				end
				...
				if table.HasValue({"sandbox", "darkrp"}, engine.ActiveGamemode()) then
					undo.Create("Объект (" .. ply:GetActiveWeapon().Fortifications[ply:GetActiveWeapon().fortificationSelection]["name"] .. ")")
	...
	hook.Remove("PostDrawTranslucentRenderables","Lobby_PostDrawOpaqueRenderables_EntityDisplays", function()
				...
				if IsValid(v) and v:GetPos():Distance(LocalPlayer():GetPos()) <= 250 then
					if v:GetClass() == "prop_physics" and v:GetNWString("alydusFortificationHologramName", false) != false then
						...
						cam.Start3D2D(pos, Angle(0, ang.y, 90), 0.10)
							draw.DrawText("Выбор Объектов", "Alydus.FortificationsTablet.Title", 0, 0, Color(255, 255, 255), TEXT_ALIGN_CENTER)
	...
	if SERVER then
		...
		if IsValid(ply) and ply:Alive() and IsValid(ply:GetActiveWeapon()) and ply:GetActiveWeapon():GetClass() == "cp_fort" and ply:GetActiveWeapon().fortificationSelection then
			if ply:GetEyeTrace().Entity.isPlayerPlacedFortification == ply then
				if ply:GetEyeTrace().Entity:GetClass() == 'ent_weapon_mg42pod' then
					ply:SetNWInt('ent_weapon_mg42pod', ply:GetNWInt('ent_weapon_mg42pod') - 1)
				elseif ply:GetEyeTrace().Entity:GetClass() == 'ent_weapon_maximpod' then
					ply:SetNWInt('ent_weapon_maximpod', ply:GetNWInt('ent_weapon_maximpod') - 1)
				elseif ply:GetEyeTrace().Entity:GetClass() == 'metahub_goammo' then
					ply:SetNWInt('metahub_goammo', ply:GetNWInt('metahub_goammo') - 1)
				end
		...
		if IsValid(vm) then
			...
			cam.Start3D2D(pos - ang:Right() * 3 - ang:Forward() * 8.25 + ang:Right() * 7.4, Angle(0, ply:EyeAngles().y, ang.z) + Angle(180, 90, 160), 0.01)
				...
				if self:GetNWBool("tabletBootup", false) == false then
					draw.SimpleText("Создание Объектов", "Alydus.FortificationsTablet.Title", 15, 0, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
					...
					draw.SimpleText("      " .. table.Count(self.Fortifications) .. " Доступно объектов", "Alydus.FortificationsTablet.Subtitle", 15, 180, Color(100, 100, 100, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				else
					draw.SimpleText("Инженерная панель", "Alydus.FortificationsTablet.Title", 15, 30, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)

					-- surface.SetMaterial(refreshMat)
					-- surface.DrawTexturedRect(-130, 70, 40, 40)

					draw.SimpleText("    Modern | Загрузка...", "Alydus.FortificationsTablet.Subtitle", 15, 90, Color(150, 150, 150, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
				end
			cam.End3D2D()
		end
	end

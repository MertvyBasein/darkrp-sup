...
if (CLIENT) then
	...
	language.Add("Tool.stacker.name", "Копирование пропов")
...
elseif (SERVER) then
	...
	local function ValidateCommand(ply, arg)
		if (IsValid(ply) and !ply:IsSuperAdmin()) then return false end
...
function TOOL:LeftClick(trace)
	...
	for i = 1, count, 1 do
		...
		if not hook.Call("PlayerSpawnProp", GAMEMODE, self:GetOwner(), entMod) then break end

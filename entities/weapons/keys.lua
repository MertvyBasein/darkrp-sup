...
if SERVER then
	LockDoor = function(self, lock)
		...
		local isteam = isnumber(ent:DoorGetTeam())
		...
		if (not IsOwner and not isteam/* and not IsHotelOwner*/) then
			...
			elseif ((not ent.NextBell) or (ent.NextBell <= CurTime())) and IsValid(ent:DoorGetOwner()) then
				rp.Notify(ent:DoorGetOwner(), NOTIFY_GENERIC, 'Кто-то позвонил в ваш звонок.')
		...
		if ent.KeysCooldown and (ent.KeysCooldown > CurTime()) then
			rp.Notify(self.Owner, NOTIFY_ERROR, 'Пожалуйста, подождите прежде чем закрывать дверь.')
			return
		end
		if IsOwner or (isteam && ent:DoorGetTeam() == self.Owner:Team()) then
			...
			ent:DoorLock(lock)
			rp.Notify(self.Owner, NOTIFY_GENERIC, lock and 'Закрыто!' or 'Открыто!')
		end
	end

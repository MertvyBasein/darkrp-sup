...
function ENT:Use(activator, caller, usetype, value)
	if caller:IsPlayer() and (not caller:IsBanned()) and ((not caller['NextUse' .. self:GetClass()]) or (caller['NextUse' .. self:GetClass()] <= CurTime())) and self:CanUse(caller) then
	...
	if self.MaxHealth then
		...
		if (self.MaxHealth <= 0) then
			...
			if IsValid(owner) then
				owner:Notify(NOTIFY_ERROR, 'Ваша # было уничтожено!', self.PrintName)
			end
		end
	end
end

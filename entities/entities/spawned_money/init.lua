...
function ENT:Initialize()
	self:SetModel("models/reichsmarks/50 reichsmarks note.mdl")
...
function ENT:Use(activator,caller)
	if activator.sBanned then return end
	local amount = self:Getamount() or 0

	activator:AddMoney(amount, 'Подбирание денег')

	hook.Call('PlayerPickupRPMoney', GAMEMODE, activator, amount, activator:GetMoney())

	DarkRP.notify(activator, NOTIFY_SUCCESS, 'Вы нашли #!', rp.FormatMoney(amount))
	self:Remove()
end
...
local ents = ents
timer.Remove('rp_moneymerge', 5, 0, function()

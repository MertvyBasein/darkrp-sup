...
function ENT:SalePrice(activator)
	...
	local discounted = math.ceil(self.MinPrice * 1.42)
...
function ENT:Use(pl)
	...
	elseif (pl == owner) then
		pl:Notify(NOTIFY_GENERIC, term.Get('BoughtFoodProduction'), rp.FormatMoney(self.MinPrice))
